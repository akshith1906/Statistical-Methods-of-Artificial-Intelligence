...
def train_tree_greedy(
    ...
    # --- Stopping conditions ---
    # 1. If we have reached max depth
    if depth >= MAX_DEPTH:
        return LeafNode(x_NF, y_N)
    # 2. If this node has too few samples to be an internal node
    elif N < MIN_SAMPLES_INTERNAL:
        return LeafNode(x_NF, y_N)
    # 3. (Optional but good): If all y values are the same
    elif np.allclose(y_N, y_N[0]):
        return LeafNode(x_NF, y_N)
    else:
        # --- Find the best split ---
        ...
        if feat_id is None:
            ...
            # (e.g., all features are identical, or no split satisfies MIN_SAMPLES_LEAF)
            return LeafNode(x_NF, y_N)
        else:
            # --- START OF SOLUTION (TASK D) ---
