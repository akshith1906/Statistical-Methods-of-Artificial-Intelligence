...
def select_best_binary_split(x_NF, y_N, MIN_SAMPLES_LEAF=1):
    ...
    for f in range(F):
        ...
        # We can efficiently check this by looking at sorted feature values.
        x_N = x_NF[:, f]
        sort_idx = np.argsort(x_N)
        x_sorted_N = x_N[sort_idx]
        
        valid_thresh_mask_V = np.ones(V, dtype=bool)
        for v_id, thresh in enumerate(possib_xthresh_V):
            n_left = np.sum(x_sorted_N < thresh)
            if n_left < MIN_SAMPLES_LEAF:
                valid_thresh_mask_V[v_id] = False
            
            n_right = N - n_left
            if n_right < MIN_SAMPLES_LEAF:
                valid_thresh_mask_V[v_id] = False
                
        possib_xthresh_V = possib_xthresh_V[valid_thresh_mask_V]
        V = possib_xthresh_V.size
        # --- End of MIN_SAMPLES_LEAF constraint ---

        if V == 0:
            # If all the x values for this feature are same, or no valid splits
        ...
        # --- START OF SOLUTION C.1 ---
        x_N = x_NF[:, f] # Get all N values for this feature
        ...
        # Loop over all valid potential thresholds
        for v in range(V):
            ...
            # This check is now redundant due to pre-filtering, but safe
            if y_L.size == 0 or y_R.size == 0:
                left_cost_V[v] = np.inf
                right_cost_V[v] = np.inf
                continue

            # Calculate the mean for each child
            y_mean_L = np.mean(y_L)
            y_mean_R = np.mean(y_R)
            
            left_yhat_V[v] = y_mean_L
            right_yhat_V[v] = y_mean_R
            
            # Cost is sum of squared error (SSE)
            left_cost_V[v] = np.sum(np.square(y_L - y_mean_L))
            right_cost_V[v] = np.sum(np.square(y_R - y_mean_R))
        ...
        if V == 0 or np.all(np.isinf(total_cost_V)):
            cost_F[f] = np.inf
            continue
            
        # If not, all splits will have same cost and we should just not split.
        costs_all_the_same = np.allclose(total_cost_V, total_cost_V[np.isfinite(total_cost_V)][0])
        ...
        # --- START OF SOLUTION C.2 ---
        chosen_v_id = np.nanargmin(total_cost_V) # Use nanargmin to ignore inf
        # --- END OF SOLUTION C.2 ---
    ...
    best_feat_id = np.nanargmin(cost_F) # Use nanargmin to ignore inf
